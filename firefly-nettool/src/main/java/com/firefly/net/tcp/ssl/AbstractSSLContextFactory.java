...
public abstract class AbstractSSLContextFactory implements SSLContextFactory {
    ...
    protected SSLContext getSSLContextWithManager(KeyManager[] km, TrustManager[] tm, SecureRandom random)
    ...
    protected SSLContext getSSLContext(InputStream in, String keystorePassword, String keyPassword)
    ...
    public SSLEngine createSSLEngine(boolean clientMode) {
        if (sslContext == null) {
            synchronized (this) {
                if (sslContext == null) {
                    createSSLContext();
                    return _createSSLEngine(clientMode);
                } else {
                    return _createSSLEngine(clientMode);
                }
            }
    ...
    private SSLEngine _createSSLEngine(boolean clientMode) {
        SSLEngine sslEngine = sslContext.createSSLEngine();
        sslEngine.setUseClientMode(clientMode);
        return sslEngine;
    }

    abstract public void createSSLContext();
