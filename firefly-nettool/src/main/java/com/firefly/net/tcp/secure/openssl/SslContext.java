...
/**
 * A secure socket protocol implementation which acts as a factory for {@link SSLEngine}.
 * It is implemented via OpenSSL's {@code SSL_CTX}.
 */
public abstract class SslContext {
    ...
    public final List<String> nextProtocols() {
        return applicationProtocolNegotiator().getSupportedApplicationProtocols();
    }

    /**
     * Returns the object responsible for negotiating application layer protocols for the TLS NPN/ALPN extensions.
     */
    public abstract ApplicationProtocolSelector applicationProtocolNegotiator();
    ...
    private static X509Certificate[] getCertificatesFromBuffers(ByteBuffer[] certs) throws CertificateException {
        CertificateFactory cf = CertificateFactory.getInstance("X.509");
        X509Certificate[] x509Certs = new X509Certificate[certs.length];

        for (int i = 0; i < certs.length; i++) {
            byte[] d = new byte[certs[i].remaining()];
            certs[i].get(d);
            try (InputStream is = new ByteArrayInputStream(d)) {
