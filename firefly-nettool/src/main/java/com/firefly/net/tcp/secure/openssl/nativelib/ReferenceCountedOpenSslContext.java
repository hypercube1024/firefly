...
import static com.firefly.net.tcp.secure.openssl.nativelib.ObjectUtil.checkPositiveOrZero;
import static com.firefly.net.tcp.secure.openssl.nativelib.OpenSsl.DEFAULT_CIPHERS;
import static com.firefly.net.tcp.secure.openssl.nativelib.OpenSsl.availableJavaCipherSuites;
...
public abstract class ReferenceCountedOpenSslContext extends SslContext implements ReferenceCounted {
    ...
    private static final boolean JDK_REJECT_CLIENT_INITIATED_RENEGOTIATION =
            AccessController.doPrivileged((PrivilegedAction<Boolean>) () -> SystemPropertyUtil.getBoolean("jdk.tls.rejectClientInitiatedRenegotiation", false));

    private static final int DEFAULT_BIO_NON_APPLICATION_BUFFER_SIZE =
            AccessController.doPrivileged((PrivilegedAction<Integer>) () -> Math.max(1, SystemPropertyUtil.getInt("io.netty.handler.ssl.openssl.bioNonApplicationBufferSize", 2048)));
    ...
    static {
        ...
        try {
            String dhKeySize = AccessController.doPrivileged((PrivilegedAction<String>) () -> SystemPropertyUtil.get("jdk.tls.ephemeralDHKeySize"));
            if (dhKeySize != null) {
                try {
    ...
    ReferenceCountedOpenSslContext(Iterable<String> ciphers, CipherSuiteFilter cipherFilter,
        ...
        try {
            ...
            List<String> nextProtoList = apn.getSupportedApplicationProtocols();
    ...
    @Override
    public ApplicationProtocolSelector applicationProtocolNegotiator() {
        return apn;
    }
    ...
    public final SSLEngine newEngine(String peerHost, int peerPort) {
        return newEngine0(peerHost, peerPort, true);
    }

    SSLEngine newEngine0(String peerHost, int peerPort, boolean jdkCompatibilityMode) {
        return new ReferenceCountedOpenSslEngine(this, peerHost, peerPort, jdkCompatibilityMode);
    }
    ...
    static void setKeyMaterial(long ctx, X509Certificate[] keyCertChain, PrivateKey key, String keyPassword)
        ...
        try {
            ...
            PemEncoded encoded = PemX509Certificate.toPEM(keyCertChain);
            keyCertChainBio = toBIO(encoded);
            keyCertChainBio2 = toBIO(encoded);
            ...
            SSLContext.setCertificateBio(
                    ...
                    keyPassword == null ? StringUtils.EMPTY : keyPassword);
