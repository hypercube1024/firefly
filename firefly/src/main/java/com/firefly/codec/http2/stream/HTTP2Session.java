...
import com.firefly.codec.common.CountingCallback;
...
public abstract class HTTP2Session implements SessionSPI, Parser.Listener {
	...
	public String toString() {
		return String.format("%s@%x{queueSize=%d,sendWindow=%s,recvWindow=%s,streams=%d,%s}",
				getClass().getSimpleName(), hashCode(), flusher.getQueueSize(), sendWindow, recvWindow, streams.size(),
				closed);
	}
