...
public class HpackEncoder {
	...
	public void encode(ByteBuffer buffer, HttpField field) {
		...
		if (entry != null) {
			...
			if (header == null) {
				...
				// has the custom header name been seen before?
					...
					// unknown name and value, so let's index this just in case
					// it is
					// the first time we have seen a custom name or a custom
					// field.
					...
					// This is probably a custom field with changing value, so
					// don't index.
					indexed = false;
				...
				if (field instanceof PreEncodedHttpField) {
					...
					if (_debug)
						encoding = "Lit"
								+ ((name == null) ? "HuffN"
										: ("IdxN" + (name.isStatic() ? "S" : "")
												+ (1 + NBitInteger.octectsNeeded(4, _context.index(name)))))
								+ (huffman ? "HuffV" : "LitV") + (indexed ? "Idx" : (never_index ? "!!Idx" : "!Idx"));
					...
					encodeValue(buffer, huffman, field.getValue());
					if (_debug)
						encoding = ((name == null) ? "LitHuffN"
								: ("LitIdxN" + (name.isStatic() ? "S" : "")
										+ (1 + NBitInteger.octectsNeeded(6, _context.index(name)))))
		...
		if (_debug) {
			...
			if (log.isDebugEnabled())
				log.debug("encode {}:'{}' to '{}'", encoding, field,
						TypeUtils.toHexString(buffer.array(), buffer.arrayOffset() + p, e - p));
		}
	}
