...
import static com.firefly.codec.http2.model.HttpTokens.CARRIAGE_RETURN;
import static com.firefly.codec.http2.model.HttpTokens.LINE_FEED;
import static com.firefly.codec.http2.model.HttpTokens.SPACE;
import static com.firefly.codec.http2.model.HttpTokens.TAB;
 ...
 * {@link #parseNext(ByteBuffer)} method. The parsed elements of the HTTP
 * message are passed as event calls to the {@link HttpHandler} instance the
 * parser is constructed with. If the passed handler is a {@link RequestHandler}
 * then server side parsing is performed and if it is a {@link ResponseHandler},
 * then client side parsing is done.
 * </p>
 * <p>
 * The contract of the {@link HttpHandler} API is that if a call returns true
 * then the call to {@link #parseNext(ByteBuffer)} will return as soon as
 ...
 * {@link #parseNext(ByteBuffer)} rather than from within the scope of a call
 * like {@link RequestHandler#messageComplete()}
 * </p>
 * <p>
 * For performance, the parse is heavily dependent on the
 * {@link Trie#getBest(ByteBuffer, int, int)} method to look ahead in a single
 * pass for both the structure ( : and CRLF ) and semantic (which header and
 * value) of a header. Specifically the static {@link HttpHeader#CACHE} is used
 * to lookup common combinations of headers and values (eg. "Connection: close"
 * ), or just header names (eg. "Connection:" ). For headers who's value is not
 * known statically (eg. Host, COOKIE) then a per parser dynamic Trie of
 * {@link HttpFields} from previous parsed messages is used to help the parsing
...
public class HttpParser {
	 ...
	 * <li>Content-Length: 0
	 * </ul>
	 * <li>Combinations of Content-Type header for common mime types by common
	 * charsets
	...
	static {
		...
		for (String type : new String[] { "text/plain", "text/html", "text/xml", "text/json", "application/json",
		...
		while (_state.ordinal() < State.HEADER.ordinal() && buffer.hasRemaining() && !handle) {
			...
			case SPACE1:
				if (ch > HttpTokens.SPACE || ch < 0) {
					...
					throw new BadMessageException(HttpStatus.BAD_REQUEST_400,
							_requestHandler != null ? "No URI" : "No Status");
				}
				break;
				...
				if (ch > HttpTokens.SPACE) {
					...
					if (_responseHandler != null) {
						...
						if (buffer.position() > 0 && buffer.hasArray())
							version = HttpVersion.lookAheadGet(buffer.array(),
					...
					if (buffer.hasRemaining()) {
						...
						if (field != null) {
							...
							if (_compliance == HttpCompliance.LEGACY) {
								...
								n = BufferUtils.toString(buffer, buffer.position() - 1, fn.length(),
										StandardCharsets.US_ASCII);
								if (fv == null)
									v = null;
								else {
									v = BufferUtils.toString(buffer, buffer.position() + fn.length() + 1, fv.length(),
	 ...
	 * @param buffer
	 *            the buffer to parse
	 * @return True if an {@link RequestHandler} method was called and it
	 *         returned true;
	 */
	public boolean parseNext(ByteBuffer buffer) {
		...
		try {
			...
			if (stack)
				log.warn("bad HTTP parsed: " + e._code + (e.getReason() != null ? " " + e.getReason() : "") + " for "
						+ _handler, e);
			else
				log.warn("bad HTTP parsed: " + e._code + (e.getReason() != null ? " " + e.getReason() : "") + " for "
						+ _handler);
	...
	public interface HttpHandler {
		 ...
		 * 
		 * @param status
		 *            The bad status to send
		 * @param reason
		 *            The textual reason for badness
		 */
	...
	public interface RequestHandler extends HttpHandler {
		 ...
		 *            The method
		 * @param uri
		 *            The raw bytes of the URI. These are copied into a
		 *            ByteBuffer that will not be changed until this parser is
		 *            reset and reused.
		 * @param version
	...
	private static class IllegalCharacterException extends BadMessageException {
		private IllegalCharacterException(State state, byte ch, ByteBuffer buffer) {
			...
			log.warn(String.format("Illegal character 0x%X in state=%s for buffer %s", ch, state,
					BufferUtils.toDetailString(buffer)));
		}
	}
}
