...
public class HTTP2ClientH2cDemo {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (connection.getHttpVersion() == HttpVersion.HTTP_1_1) {
            ...
            httpConnection.upgradeHTTP2(request, settingsFrame, http2promise, initStream,
            ...
            if (connection2.getHttpVersion() == HttpVersion.HTTP_2) {
                ...
                clientStream.data(smallDataFrame, new Callback() {
                    ...
                    public void succeeded() {
                        System.out.println("client sends small data success");
                        clientStream.data(bigDataFrame, new Callback() {

                            @Override
                            public void succeeded() {
                                System.out.println("client sends big data success");
                            }

                            @Override
                            public void failed(Throwable x) {
                                log.info("client sends big data failure");
                            }
                        });
                    }
                    ...
                    @Override
                    public void failed(Throwable x) {
                        log.info("client sends small data failure");
                    }
                });
