...
public class HttpGeneratorServerHTTPTest {
	...
	public void testHTTP() throws Exception {
		...
		if (run.httpVersion == 10)
			assertTrue(t, gen.isPersistent() || run.result._contentLength >= 0 || EnumSet
					.of(ConnectionType.CLOSE, ConnectionType.KEEP_ALIVE, ConnectionType.NONE).contains(run.connection));
		else
	...
	private static class Result {
		...
		private String build(int version, HttpGenerator gen, String reason, String connection, String te, int nchunks)
			...
			loop: while (true) {
				...
				switch (result) {
				case NEED_INFO:
					info = new MetaData.Response(HttpVersion.fromVersion(version), _code, reason, _fields,
							_contentLength);
					continue;
	...
	private static class Run {
		...
		@Override
		public String toString() {
			return String.format("result=%s,version=%d,chunks=%d,connection=%s", result, httpVersion, chunks,
					connection.name());
		}
	}

	private enum ConnectionType {
		NONE(null, 9, 10, 11), KEEP_ALIVE("keep-alive", 9, 10, 11), CLOSE("close", 9, 10, 11), TE_CLOSE("TE, close",
				11);
	...
	public static Collection<Run[]> data() {
		Result[] results = { new Result(200, null, -1, null, false), new Result(200, null, -1, CONTENT, false),
				new Result(200, null, CONTENT.length(), null, true),
				new Result(200, null, CONTENT.length(), CONTENT, false), new Result(200, "text/html", -1, null, true),
