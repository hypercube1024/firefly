...
public class URLEncodedTest {
	...
	static {
		/*
		 * Uncomment to set setting the System property to something other than
	...
	public void testUrlEncoded() {
		...
		assertTrue("multi encode " + url_encoded.encode(), url_encoded.encode().equals("Name5=aaa&Name6=bbb")
		...
		if (java.nio.charset.Charset.isSupported("SJIS")) {
			url_encoded.clear();
			url_encoded.decode("Name9=%u30ED%83e%83X%83g", Charset.forName("SJIS")); // "Test"
																						// in
																						// Japanese
																						// Katakana
			assertEquals("encoded param size", 1, url_encoded.size());
	...
	@Test
	public void testUrlEncodedStream() throws Exception {
		String[][] charsets = new String[][] { { "utf-8", null, "%30" }, { "iso-8859-1", "iso-8859-1", "%30" },
				{ "utf-8", "utf-8", "%30" }, { "utf-16", "utf-16", "%00%30" }, };
		...
		if (java.nio.charset.Charset.isSupported("Shift_JIS")) {
			ByteArrayInputStream in2 = new ByteArrayInputStream(
					"name=%83e%83X%83g".getBytes(StandardCharsets.ISO_8859_1));
	...
	public void testCharsetViaSystemProperty() throws Exception {
		 ...
		 * 
		 * 
		 * ByteArrayInputStream in3 = new
		 * ByteArrayInputStream("name=libell%E9".getBytes(StringUtil.
		 * __ISO_8859_1)); MultiMap m3 = new MultiMap();
		 * UrlEncoded.decodeTo(in3, m3, null, -1); assertEquals("stream name",
		 * "libell\u00E9", m3.getString("name"));
		 * 
		 */
	}
	...
	public void testNotUtf8() throws Exception {
		...
		System.out.println("EXPECT 4 Not Valid UTF8 warnings...");
