 ...
 * </p>
 * <p>
 * This Trie is of a fixed size and cannot grow (which can be a good thing with
 ...
 * <dl>
 * <dt>char[] _tree</dt>
 * <dd>This is semantically 2 dimensional array flattened into a 1 dimensional
 * char array. The second dimension is that every 4 sequential elements
 * represents a row of: character; hi index; eq index; low index, used to build
 * a ternary trie of key strings.</dd>
 * <dt>String[] _key</dt>
 * <dd>An array of key values where each element matches a row in the _tree
 * array. A non zero key element indicates that the _tree row is a complete key
 * rather than an intermediate character of a longer key.</dd>
 * <dt>V[] _value</dt>
 * <dd>An array of values corresponding to the _key array</dd>
 * </dl>
 * <p>
 * The lookup of a value will iterate through the _tree array matching
 * characters. If the equal tree branch is followed, then the _key array is
 * looked up to see if this is a complete match. If a match is found then the
 * _value array is looked up to return the matching value.
 * </p>
 * <p>
...
public class ArrayTernaryTrie<V> extends AbstractTrie<V> {
	...
	/**
	 * The Size of a Trie row is the char, and the low, equal and high child
	 * pointers
	 */
	private static final int ROW_SIZE = 4;

	/**
	 * The Trie rows in a single array which allows a lookup of row,character to
	 * the next row in the Trie. This is actually a 2 dimensional array that has
	 * been flattened to achieve locality of reference.
	 */
	private final char[] _tree;

	/**
	 * The key (if any) for a Trie row. A row may be a leaf, a node or both in
	 * the Trie tree.
	 */
	private final String[] _key;

	/**
	 * The value (if any) for a Trie row. A row may be a leaf, a node or both in
	 * the Trie tree.
	 */
	 ...
	 * 
	 * @param capacity
	 *            The capacity of the Trie, which is in the worst case is the
	 *            total number of characters of all keys stored in the Trie. The
	 *            capacity needed is dependent of the shared prefixes of the
	 *            keys. For example, a capacity of 6 nodes is required to store
	 *            keys "foo" and "bar", but a capacity of only 4 is required to
	 *            store "bar" and "bat".
	 */
	...
	public V getBest(ByteBuffer b, int offset, int len) {
		if (b.hasArray())
			return getBest(0, b.array(), b.arrayOffset() + b.position()
					+ offset, len);
	...
	public void dump() {
		for (int r = 0; r < _rows; r++) {
			...
			System.err.printf("%4d [%s,%d,%d,%d] '%s':%s%n", 
					r,
					(c < ' ' || c > 127) ? ("" + (int) c) : "'" + c + "'",
					(int) _tree[r * ROW_SIZE + LO], (int) _tree[r * ROW_SIZE + EQ], (int) _tree[r * ROW_SIZE + HI], _key[r],
					_value[r]);
		}
