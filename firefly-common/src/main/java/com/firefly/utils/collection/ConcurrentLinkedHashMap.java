...
public class ConcurrentLinkedHashMap<K, V> implements Map<K, V> {
    ...
    public ConcurrentLinkedHashMap(boolean accessOrder, int maxEntries) {
        this(accessOrder, maxEntries, new MapEventListener<K, V>(){
            ...
            @Override
            public V onGetEntry(K key, V value) {
                return value;
            }

            @Override
            public V onPutEntry(K key, V value, V previousValue) {
                return previousValue;
            }

            @Override
            public V onRemoveEntry(K key, V value) {
                return value;
            }
        });
    }
    ...
    static final class LinkedHashMapSegment<K, V> extends LinkedHashMap<K, V> {
        ...
        protected boolean removeEldestEntry(Map.Entry<K, V> eldest) {
            lock.lock();
            try {
                return size() > maxEntries && mapEventListener.onEliminateEntry(eldest.getKey(), eldest.getValue());
            } finally {
                lock.unlock();
            }
        }
     ...
     * @return the segment
     */
    private LinkedHashMapSegment<K, V> segmentFor(int hash) {
        int h = hash(hash);
    ...
    public String toString() {
        ...
        for (int i = 0; i < segments.length; i++) {
            s.append("segment ").append(i).append(" -> ").append(segments[i].toString());
            s.append("\r\n");
        }
        return s.toString();
    }
